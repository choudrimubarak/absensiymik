...
<?php
    ...
    $view2=mysqli_query($conn, "select tb_jadwal.id_kelas, tb_kelas.kelas from tb_jadwal, tb_kelas 
    ...
    $view4=mysqli_query($conn, "select 
                            tb_absensih.tanggal, 
                            tb_absensih.nis,
                            tb_siswa.id_kelas
                        from 
    ...
    <div class="col-lg-8">
        ...
        <form method="post">
            <div class="table-responsive">
                <?php
                    $view3=mysqli_query($conn, "select * from tb_siswa where id_kelas='$kelasid'");
                    $no=0;
                ?>
    ...
    <?php
        if (@$_POST['input']){
            ...
            for($i=0;$i<$no;$i++){
                ...
                $query=mysqli_query($conn, "insert into tb_absensi(id_jadwal, tanggal, nis, ket) values('$id_jadwal','$tanggal','$nis','$ket')") or die (mysqli_error());
            ...
            if($cek==0){
                for($a=0;$a<$no;$a++){
                    ...
                    $query1=mysqli_query($conn, "insert into tb_absensih(tanggal, nis, ket) values('$tanggal','$nis','$ket')") or die (mysqli_error());
                }
            }
